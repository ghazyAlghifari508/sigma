...
interface LaporanClientProps {
  sppgTerdekat: { id: string; namaDapur: string; kabupaten: string }[];
  namaSekolah: string;
}

export default function LaporanClient({ sppgTerdekat, namaSekolah }: LaporanClientProps) {
  const [isSubmitted, setIsSubmitted] = useState(false);

  const form = useForm<z.infer<typeof formSchema>>({
    resolver: zodResolver(formSchema),
    defaultValues: {
      sppgId: sppgTerdekat.length === 1 ? sppgTerdekat[0].id : undefined,
      judul: "",
      deskripsi: "",
    },
  });
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        <div>
          <h2 className="text-[32px] md:text-[40px] font-normal text-ink mb-2 flex items-center gap-2 tracking-tight">
            <FileWarning className="h-7 w-7 text-indigo-600" />
            Form Keluhan Siswa
          </h2>
          <p className="text-body">
            Sampaikan keluhan atau masukan kamu terkait program Makan Bergizi Gratis (MBG) di <strong className="text-indigo-600">{namaSekolah}</strong>. Laporan akan diteruskan ke petugas dapur yang dipilih.
          </p>
        </div>
      </div>
            ...
            <Form {...form}>
              <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel className="text-ink">Pilih SPPG (Satu Provinsi)</FormLabel>
                      {sppgTerdekat.length === 0 ? (
                        <div className="p-3 bg-amber-50 text-amber-700 border border-amber-200 rounded-md text-sm">
                          Peringatan: Belum ada SPPG yang terdaftar di provinsi Anda. Laporan akan ditampung oleh Pemerintah.
                        </div>
                      ) : (
                        <Select onValueChange={field.onChange} value={field.value ?? ""}>
                          <FormControl>
                            <SelectTrigger className="w-full bg-surface-soft border-hairline text-ink focus:ring-indigo-600">
                              <SelectValue placeholder="Pilih SPPG yang melayani sekolah Anda">
                                {field.value ? sppgTerdekat.find(s => s.id === field.value)?.namaDapur : "Pilih SPPG yang melayani sekolah Anda"}
                              </SelectValue>
                            </SelectTrigger>
                          </FormControl>
                          <SelectContent className="bg-[#fffbf7] border-hairline text-ink">
                            {sppgTerdekat.map((sppg) => (
                              <SelectItem key={sppg.id} value={sppg.id}>
                                {sppg.namaDapur} ({sppg.kabupaten})
                              </SelectItem>
                            ))}
                          </SelectContent>
                        </Select>
                        ...
                        <FormControl>
                          <SelectTrigger className="w-full bg-surface-soft border-hairline text-ink focus:ring-indigo-600">
                            <SelectValue placeholder="Pilih jenis keluhan">
                              {field.value === "Keterlambatan" ? "Keterlambatan Distribusi" : 
                               field.value === "Kualitas" ? "Kualitas Makanan" : 
                               field.value === "Porsi" ? "Porsi Tidak Sesuai" : 
                               field.value === "Lainnya" ? "Lainnya" : 
                               "Pilih jenis keluhan"}
                            </SelectValue>
                          </SelectTrigger>
                        </FormControl>
                        <SelectContent className="bg-[#fffbf7] border-hairline text-ink min-w-[var(--radix-select-trigger-width)]">
